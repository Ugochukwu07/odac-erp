...
{
	return base_url('private/' . $path);
}

function adminfold($path)
{
	return ('private/' . $path);
}
...
if (!function_exists('fetchall')) {
	function fetchall($table, $where, $keys = null, $orderby = null)
	{
		if (!is_null($keys) && !empty($keys)) {
			$row = ci()->db->select($keys);
		}
		if (!is_null($orderby)) {
			$row = ci()->db->order_by($orderby);
		}
...
/**
 * Perform a cURL request to an external or local API.
 *
 * This function supports both external and local URLs (e.g., http://odac.local).
 * It handles GET and POST methods, allows custom headers, and sets appropriate cURL options
 * for local development environments (such as disabling SSL verification only for HTTPS).
 *
 * @param string $url     The API endpoint URL.
 * @param string $method  The HTTP method ('GET' or 'POST').
 * @param array|null $array   The data to send with the request (for POST).
 * @param array|null $header  Optional headers to send.
 * @param int|null $time      Optional timeout in seconds.
 * @return array|null         The decoded JSON response as an associative array, or null on failure.
 */
if (!function_exists('curl_apis')) {
	/**
	 * CodeIgniter cURL API helper.
	 *
	 * @param string $url
	 * @param string $method
	...
	function curl_apis($url, $method, $array = null, $header = null, $time = null)
	{
		...
		$jsonstring = null;
		if ($method === 'POST' && !empty($array)) {
			$jsonstring = json_encode($array);
		}

		$curl = curl_init();

		// Set timeout (default 50 seconds)
		$timeout = !is_null($time) ? $time : 50;
		...
		// Set headers if provided, else set default JSON header for POST
		if (!is_null($header)) {
		...
		// Handle SSL verification for HTTPS URLs only
		...
		if ($jsondata === false) {
			log_message('error', 'cURL error: ' . curl_error($curl) . ' for URL: ' . $url);
			curl_close($curl);
			// Return a detailed error array for debugging instead of null
			return array(
				'status' => false,
		...
		// Try to decode JSON, if fails, return raw response for debugging
		$data = json_decode($jsondata, true);
		...
		// If $data is null or not an array, return raw response for debugging
		if (empty($data) && !empty($jsondata)) {
			return array(
				'status' => false,
				'error' => 'Empty or invalid decoded data',
				'raw_response' => $jsondata,
				'url' => $url
			);
		}
...
function sendmail($to, $subject, $message, $file = null, $replyto = null)
{
	...
	if ($ip != '127.0.0.1') {
		...
		if (!is_null($replyto)) {
			ci()->email->reply_to($replyto, $mailer);
		}
...
if (!function_exists('termlist')) {
	function termlist($datacategory = null, $contenttype = null)
	{
		...
		if (!is_null($datacategory)) {
			$wherear['datacategory'] = $datacategory;
		}
		if (!is_null($contenttype)) {
			$wherear['contenttype'] = $contenttype;
		}
...
function calculatePrincipal(int $amount) {
	$gst = CABGST;
