        ...
        <div class="col-md-12">
            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                    ...
                    <a href="<?php echo adminurl('vehicle_service_list?status='.$redirect );?>" class="btn btn-sm btn-success"><i class="fa fa-table"></i> View </a>  
                    </div>
                </div>
                <!-- /.box-header -->
<div class="box-body">
    <?php echo form_open_multipart( adminurl('Add_veh_service/savedata' )); ?>
...
<?php if( VIEW ==='yes'){ 
echo anchor( adminurl('vehicle_service_list?status='.$redirect  ),'Go Back',array('class'=>'btn btn-warning fa fa-sign-out'));}?>
</div>
</div>
                                                <?php echo form_close(); ?>
    ...
    function fillkm( vl ){ 
        $('#next_service_km, #tyre_alignment_km').val( ( parseInt(vl) + 3000) );
    } 
  ...
  function filldatemonth( vl ){ 
      ...
      $.ajax({
          type:'POST',
          url: '<?=base_url('private/Add_veh_service/addMonthDates');?>',
          data:{'indate':vl},
          success: (res)=>{ 
    ...
    function loadVehicleData( id ){
        let is_editMode = '<?=$id;?>';
        if( is_editMode === '' && id ){
            window.location.href='<?=base_url('private/add_veh_service?')?>redirect=<?=$redirect?>&vehicle_con_id='+id;
        } 
    }
