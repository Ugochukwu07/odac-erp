...
@media(max-width:667px){
    .store-offer-item .store-thumb-link { float: left; width: 41%; }
    .store-offer-item .store-thumb-link .offer-image {max-width: 126px; line-height: 0px;margin-bottom: 5px;text-align: center;float: left; }
    .offer-text { width: 91px; height: 86px; font-size: 17px;line-height: 24px;}
    .store-offer-item .latest-coupon {float: left;width: 58%; }
    .store-offer-item .store-thumb-link { margin-right: 0px;}
    .store-offer-item .latest-coupon { margin-right: 0px; }
    .store-offer-item .coupon-detail { float: left;margin-top: 25px;}
}
</style>
<section class="termsbg">
    <div class="container">
    <div class="row">
    <div class="col-lg-12 col-xs-12 col-md-12 col-sm-12" >
    <h2 class="title">Today Offers</h2>
    </div>
    </div>
    </div>
    </section>
...
<div class="container blue-bg">
  ...
  <div class="col-lg-12 col-xs-12 col-md-12 col-sm-12" > 
    <div class="coupons-list">
      ...
      <!-- Append coupon -->
      <?php if(!empty($cpnlist)){ foreach( $cpnlist as $key=>$row ){ 
          $topic = '';
        if( $row['valuetype'] == 'fixed' ){
            $topic = 'Get <br/>Flat ₹'.$row['cpnvalue']. ' <br/> OFF';
        }else if( $row['valuetype'] == 'percent' ){
            $topic = 'Get <br/> '.$row['cpnvalue']. ' % <br/> OFF';
        }
      ?>
      ...
      <div class="store-offer-item shadow-box">
        ...
        <div class="latest-coupon">
          <h3 class="coupon-title"><a href="#"><?=$row['titlename'];?></a></h3>
          <div class="coupon-des">
            <?=$row['description'];?>
          </div>
        </div>
        <div class="coupon-detail coupon-button-type">
          <a href="<?=base_url('')?>" class="coupon-button coupon-code" data-aff-url="<?=base_url('today-offers.php')?>">
            <span class="code-text"><?=strtoupper($row['couponcode'])?></span>
            <span class="get-code">Get Code</span>
          </a>
          <div class="store-name">
            On <?=ucwords($row['trippackage']);?> Rental 
          </div>
          <div class="exp-text">Expires on <?=date('d/m/Y',strtotime($row['validto']))?> </div>
        </div>
