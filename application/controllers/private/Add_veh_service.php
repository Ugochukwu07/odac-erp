...
class Add_veh_service extends CI_Controller{
	var $pagename;
	var $table;
	 function __construct() {
         parent::__construct();   
	     adminlogincheck();
	     $this->pagename = 'Add_veh_service';
  ...
  public function index() { 
	...
	$data['title'] = 'Add Bike Service Details';
	...
	if( $id ){
     $old_dta = $this->c_model->getSingle($table,['md5(id)'=>$id],'*');
	   ...
	   $oldData = $this->db->query("select * from ".$table." where vehicle_con_id = '".$vehicle_con_id."' AND catid = 2 ORDER BY id DESC LIMIT 1 ")->row_array(); 
	   if(!empty($oldData)){
	       	$data['service_km'] = $oldData['next_service_km']; 
	        $data['next_service_km'] = (int)$oldData['next_service_km'] + 3000;
	   }
	}
 ...
 function addMonthDates(){
   ...
   public function getEditLog(){
		    ...
		    $getData = $this->db->query('SELECT *  FROM pt_vehicle_services_editing_log WHERE table_id = '.$id.' ')->result_array();
	...
	public function verifyEditLog(){
	    $id = $this->input->post('id');
	    if( !empty($id)){
	    $this->c_model->saveupdate('pt_vehicle_services',['edit_verify_status'=>'verify'],null, ['id'=>$id]);
	    }
