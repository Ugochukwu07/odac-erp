...
class Addcommonpage extends CI_Controller{
	
	 function __construct() {
         parent::__construct(); 
	     adminlogincheck();
	     $this->pagename = 'Addcommonpage';
      }
  ...
  public function index() {
	...
	$data['title'] = ( !empty($id) ? 'Update' : 'Add') .' SEO Page For { '.checkdomain('domain').' }';
		...
		$data['status'] = 'yes';
		$data['pageurl'] = '';
		$data['domaiid'] = checkdomain('domainid');  
...
$this->form_validation->set_rules('content','content','required' ); 
	 ...
	 if( $this->form_validation->run() != false){
	    ...
	    $spost['domaiid'] = trim($dbpost['domaiid']);
		...
		$spost['pageurl'] = trim($dbpost['pageurl']);  
		...
		$checkpost['pageurl'] = trim($dbpost['pageurl']);
		$checkpost['domaiid'] = trim($dbpost['domaiid']);
    ...
    $checkpost['domaiid'] = checkdomain('domainid');   
	echo $checkitem = $this->c_model->countitem( $table, $checkpost );
